...
class Raise_Prices_With_Time_For_Woocommmerce_Admin {
	...
	public function wc_product_prices() {
        ...
        $use_new_layout_next_prices = RPT_WC_Meta::get( $product_object->get_id(), 'rpt_new_layout_next_prices' );
		$rpt_name      = 'rpt_wc';
	...
	public function wc_product_save( $post_id, $post ) {
		...
		if ( $prices ) {
            ...
            as_unschedule_all_actions( 'rpt_wc_increase_price', array( $post_id, $price ) );
		}
        ...
        if ( isset( $_POST['rpt_new_layout_next_prices'] ) ) {
            RPT_WC_Meta::update( $post_id, 'yes', 'rpt_new_layout_next_prices' );
        } else {
            RPT_WC_Meta::delete( $post_id, 'rpt_new_layout_next_prices' );
        }

...
class Raise_Prices_With_Time_For_Woocommmerce_Public {
	...
	public function show_product_countdown( $product ) {
		...
		if( $found_new_timestamp ) {
            ...
            if ( $new_layout ) {
                $next_prices = RPT_WC_Meta::get( $product_id, 'rpt_new_layout_next_prices' );
                if ( 'yes' === $next_prices ) :
                    $count = 0;
                    echo '<div class="rpt-countdown-containers">';
                    foreach ( $timestamps as $timestamp => $price ) {
                        if ( ! $count ) {
                            $count++;
                            continue;
                        }
                        echo '<div id="rpt-countdown-container-' . $timestamp . '" class="rpt-countdown-container ' . ( $count > 5 ? 'rpt-countdown-container-hidden' : '' ) .  ' ' . ( $new_layout ? 'new-layout' : '' ) . '">';
                        echo '<div class="rpt-countdown no-expiry-function ' . ( $new_layout ? 'new-layout' : '' ) . '" data-timestamp="' . $timestamp . '" data-timezone="' . get_option('gmt_offset') . '"></div>';
                        echo $price;
                        echo '</div>';
                        $count++;
                    }
